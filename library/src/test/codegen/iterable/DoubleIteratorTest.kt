...
import su.jfdev.ekollections.util.Primitives.randomDouble
...
class DoubleIteratorTest: SubjectSpek<DoubleIterator>(receiving {
    val SIZE = 100
    val list = DoubleLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomDouble())
        }
    }
    subject { list.doubleIterator() }
