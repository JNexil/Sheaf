...
import su.jfdev.ekollections.util.Primitives.randomFloat
...
class FloatIteratorTest: SubjectSpek<FloatIterator>(receiving {
    val SIZE = 100
    val list = FloatLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomFloat())
        }
    }
    subject { list.floatIterator() }
