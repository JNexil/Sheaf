...
import su.jfdev.ekollections.util.Primitives.randomLong
...
class LongIteratorTest: SubjectSpek<LongIterator>(receiving {
    val SIZE = 100
    val list = LongLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomLong())
        }
    }
    subject { list.longIterator() }
