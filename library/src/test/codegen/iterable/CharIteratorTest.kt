...
import su.jfdev.ekollections.util.Primitives.randomChar
...
class CharIteratorTest: SubjectSpek<CharIterator>(receiving {
    val SIZE = 100
    val list = CharLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomChar())
        }
    }
    subject { list.charIterator() }
