...
import su.jfdev.ekollections.util.Primitives.randomInt
...
class IntIteratorTest: SubjectSpek<IntIterator>(receiving {
    val SIZE = 100
    val list = IntLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomInt())
        }
    }
    subject { list.intIterator() }
