...
import su.jfdev.ekollections.util.Primitives.randomByte
...
class ByteIteratorTest: SubjectSpek<ByteIterator>(receiving {
    val SIZE = 100
    val list = ByteLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomByte())
        }
    }
    subject { list.byteIterator() }
