...
import su.jfdev.ekollections.util.Primitives.randomShort
...
class ShortIteratorTest: SubjectSpek<ShortIterator>(receiving {
    val SIZE = 100
    val list = ShortLists.mutable.empty().apply {
        repeat(SIZE) {
            add(randomShort())
        }
    }
    subject { list.shortIterator() }
